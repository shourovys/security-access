...
import { ackReportApi } from '../../../api/urls'
...
import AckReportTableRow from './AckReportTableRow'
  ...
  ({ order, orderBy, tableHead, apiQueryStringWithOutPagination }, ref) => {
    ...
    return (
      <TableContainer ref={ref} shadow={false}>
        <h1 className="text-2xl font-medium text-center mb-4">ACK Report</h1>
        <Table>
          <TableHeader
            order={order}
            orderBy={orderBy}
          ...
          <tbody>
            {!isLoading && (
              <>
                {data?.data.map((row) => <AckReportTableRow key={row.FormatNo} row={row} />)}
...
export default AckReportPrintModal
