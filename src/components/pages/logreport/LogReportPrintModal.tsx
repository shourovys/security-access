...
import { logReportApi } from '../../../api/urls'
...
import LogReportTableRow from './LogreportTableRow'
  ...
  ({ order, orderBy, tableHead, apiQueryStringWithOutPagination }, ref) => {
    ...
    return (
      <TableContainer ref={ref} shadow={false}>
        <h1 className="text-2xl font-medium text-center mb-4">Log Report</h1>
        <Table>
          <TableHeader
            order={order}
            orderBy={orderBy}
          ...
          <tbody>
            {!isLoading && (
              <>
                {data?.data.map((row) => <LogReportTableRow key={row.FormatNo} row={row} />)}
...
export default LogReportPrintModal
