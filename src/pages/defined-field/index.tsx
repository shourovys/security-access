...
import DefinedFieldTableRow from '../../components/pages/definedField/DefinedFieldTableRow'
import DefinedFieldTableToolbar from '../../components/pages/definedField/DefinedFieldTableToolbar'
...
import {
  IDefinedFieldApiQueryParams,
  IDefinedFieldFilters,
  IDefinedFieldResult,
} from '../../types/pages/definedField'
import { addIcon, deleteIcon, filterIcon } from '../../utils/icons'
import t from '../../utils/translator'

const TABLE_HEAD: ITableHead[] = [
  { id: 'FieldNo', label: t`Field No`, filter: true },
  { id: 'FieldName', label: t`Field Name`, filter: true },
  { id: 'FieldEnable', label: t`Field Enable`, filter: true },
  { id: 'ListEnable', label: t`List Enable`, filter: true },
  { id: 'FilterEnable', label: t`Filter Enable`, filter: true },
]

function DefinedField() {
  ...
  const initialFilterState: IDefinedFieldFilters = {
    Apply: false,
    FieldNo: '',
    FieldName: '',
  }
  ...
  const updateFilterStateToQuery = () => {
    ...
    updateRouteQueryWithReplace({
      ...
      query: {
        Page: 1,
        FieldNo: filterStateRef.current.FieldNo,
        FieldName: filterStateRef.current.FieldName,
      },
    })
  }
  ...
  useEffect(() => {
    ...
    const queryState: IDefinedFieldFilters = {
      FieldNo: typeof queryParse.FieldNo === 'string' ? queryParse.FieldNo : '',
      FieldName: typeof queryParse.FieldName === 'string' ? queryParse.FieldName : '',
      Apply: true,
    }
  ...
  const apiQueryParams: IDefinedFieldApiQueryParams = {
    offset: (page - 1) * rowsPerPage,
    limit: rowsPerPage,
    sort_by: orderBy,
    order,
    ...(filterStateRef.current.FieldNo && {
      FieldNo: filterStateRef.current.FieldNo,
    }),
    ...(filterStateRef.current.FieldName && {
      FieldName_icontains: filterStateRef.current.FieldName,
    }),
  ...
  const { isLoading, data, mutate } = useSWR<IListServerResponse<IDefinedFieldResult[]>>(
    definedFieldApi.list(apiQueryString)
  )
  ...
  const { trigger: multipleDeleteTrigger } = useSWRMutation(
    definedFieldApi.deleteMultiple,
  ...
  const breadcrumbsActions: IActionsButton[] = [
    ...
    },
    {
      icon: addIcon,
      text: t`Add`,
      link: routeProperty.definedFieldCreate.path(),
    },
    {
      icon: deleteIcon,
      text: t`Delete`,
          ...
          <TableHeader
            ...
            selectAllRow={(isAllSelected: boolean) => {
              if (data?.data) {
                handleSelectAllRow(
                  isAllSelected,
                  data?.data.map((result) => result.FieldNo.toString())
                )
              }
            }}
