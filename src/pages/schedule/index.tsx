...
function Schedule() {
  ...
  const {
    ...
    handleSelectRow,
    handleSelectAllRow,
  } = useTable({ defaultOrderBy: 'ScheduleNo' })
  ...
  const { isLoading, data, mutate } = useSWR<IListServerResponse<IScheduleResult[]>>(
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (requestData.ids.length) {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/works/schedulesmessage/?schedule_ids=${encodeURIComponent(
          jsonString
        )}`,
        {
          headers: {
