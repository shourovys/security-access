...
import { IReaderFormData, IReaderResult } from '../../../types/pages/reader'
...
function EditReader() {
  ...
  const { isLoading, data } = useSWR<ISingleServerResponse<IReaderResult>>(
    queryId ? readerApi.details(queryId) : null
  )
  useEffect(() => {
    // Set the form data to the fetched data once it's available
    if (data) {
      const { ReaderName, ReaderDesc } = data.data
      setFormData({
        ReaderName,
        ReaderDesc,
      })
  ...
  const { trigger, isMutating } = useSWRMutation(readerApi.edit(queryId), sendPutRequest, {
    onSuccess: () => {
      editSuccessfulToast()
      navigate(routeProperty.readerInfo.path(queryId))
    },
  ...
  const breadcrumbsActionsButtons: IActionsButton[] = [
    ...
    },
    {
      ...
      icon: cancelIcon,
      text: t`Cancel`,
      link: routeProperty.readerInfo.path(queryId),
    },
  ]
  ...
  return (
    <Page>
      ...
      {/* <FormActionButtonsContainer>
        ...
        <Button size="large" color="cancel" link={routeProperty.readerInfo.path(queryId)}>
