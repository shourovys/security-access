...
// Component to edit a BackupSchedule
function EditBackupSchedule() {
  ...
  const { isLoading, data } = useSWR<ISingleServerResponse<IBackupScheduleResult>>(
    queryId ? backupScheduleApi.details(queryId) : null
  )
  useEffect(() => {
    if (data) {
      // Set the form data to the fetched data once it's available
      const { Schedule, BackupName, BackupDesc, Media, BackupData }: IBackupScheduleResult =
        data.data
  ...
  const { trigger, isMutating } = useSWRMutation(backupScheduleApi.edit(queryId), sendPutRequest, {
    onSuccess: () => {
      editSuccessfulToast()
      navigate(routeProperty.backupScheduleInfo.path(queryId))
    },
  ...
  const breadcrumbsActionsButtons: IActionsButton[] = [
    ...
    },
    {
      ...
      icon: cancelIcon,
      text: t`Cancel`,
      link: routeProperty.backupScheduleInfo.path(queryId),
    },
  ]

  return (
    <Page title={t`Edit Backup Schedule`}>
      ...
      {/* <FormActionButtonsContainer>
        ...
        <Button size='large' color='cancel' link={routeProperty.backupScheduleInfo.path(queryId)}>
