...
// Component to edit a Lockset
function EditLockset() {
  ...
  const { isLoading, data } = useSWR<ISingleServerResponse<ILocksetResult>>(
    queryId ? locksetApi.details(queryId) : null
  )

  useEffect(() => {
    if (data) {
      const { Name, LocksetName, LocksetDesc, LinkId, Model, DeviceId, Partition, Gateway } =
        data.data
      setFormData({
        Name,
        LocksetName,
  ...
  const { trigger, isMutating } = useSWRMutation(locksetApi.edit(queryId), sendPutRequest, {
    onSuccess: () => {
      editSuccessfulToast()
      navigate(routeProperty.locksetInfo.path(queryId))
    },
  ...
  const breadcrumbsActionsButtons: IActionsButton[] = [
    ...
    },
    {
      ...
      icon: cancelIcon,
      text: t`Cancel`,
      link: routeProperty.locksetInfo.path(queryId),
    },
  ]
  ...
  return (
    <Page>
      ...
      {/* <FormActionButtonsContainer>
        ...
        <Button size="large" color="cancel" link={routeProperty.locksetInfo.path(queryId)}>
